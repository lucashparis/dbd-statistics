...
**Seeded with 44 killers and 46 survivors.** Run `npm run db:seed` to repopulate. The seed upserts by `name` and only sets `name`/`imageUrl`. **The killer array is mirrored from the production DB** (personalized names + local `/public/images/killers/*.webp`), so re-seeding is idempotent against the real data — do **not** point the images back at wiki URLs. Survivor images come from `static.wikia.nocookie.net`. Remote hosts are allowlisted in `next.config.ts`.

> **⚠️ Renaming a killer: the seed is NOT a migration.** `prisma/seed.ts` upserts `where: { name }`, and deploy only runs `prisma generate && next build` — it **never** executes the seed. So editing a `name` in the seed and deploying changes nothing in the DB; running `npm run db:seed` afterwards is worse — it **creates a new row** under the new name and leaves the old one orphaned with every `Match` still attached to it. To rename a killer (or repoint its `imageUrl`), `UPDATE` the **existing row by `id`** in **both** Neon branches (dev and the `main`/prod branch commented in `.env`), then edit the seed to match. Keep seed and DB byte-identical: a name present in the seed but absent from the DB is a duplicate waiting for the next `db:seed`. Changing `imageUrl` has the same trap — shipping a new `.webp` to `/public` does not update the row, which keeps rendering the placeholder.
...
> Exception (supersedes audit B11): killer names are **user-owned roster data**, not translatable UI copy. The seed mirrors the production roster verbatim, which intentionally carries Portuguese and personalized names (e.g. "Trapper (Caçador)", "Trickster (Trapaça)", "Houndmaster (Mestra da Matilha)"). Never rename a roster row on your own initiative — only when the user explicitly asks. This rule applies to static UI copy, not to roster rows.
